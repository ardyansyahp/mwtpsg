...
class FinishGoodInDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($filterPart) {
            ...
            if ($selectedPart) {
                ...
                // Fallback jika ID tidak ketemu (jarang terjadi)
                $query->where('part_id', $filterPart);
            }
        }

        // 1. Statistik Periode
        $todayQty = (clone $query)->whereDate('waktu_scan', Carbon::today())->sum('qty');
        
        $weekQty = (clone $query)->whereBetween('waktu_scan', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])->sum('qty');
        
        $monthQty = (clone $query)->whereMonth('waktu_scan', Carbon::now()->month)
        ...
        $dailyData = (clone $query)
            ...
            ->selectRaw('DATE(waktu_scan) as date, SUM(qty) as total')
            ->groupBy('date')
            ->pluck('total', 'date');
        ...
        return view('dashboard.fgin', compact(
