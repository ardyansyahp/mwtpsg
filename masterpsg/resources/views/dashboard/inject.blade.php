...
<div class="p-6 space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Dashboard Inject</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        {{-- Part di Inject Saat Ini --}}
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 text-sm font-medium">Part di Inject</p>
                    <h3 class="text-3xl font-bold mt-2">{{ number_format($currentParts) }}</h3>
                    <p class="text-purple-100 text-xs mt-1">part (In - Out)</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-green-100 text-sm font-medium">Inject In Hari Ini</p>
                    <h3 class="text-3xl font-bold mt-2">{{ $todayInjectIn }}</h3>
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-lg shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-orange-100 text-sm font-medium">Inject Out Hari Ini</p>
                    <h3 class="text-3xl font-bold mt-2">{{ $todayInjectOut }}</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6" style="height: 400px; display: flex; flex-direction: column;">
            <h3 class="text-lg font-semibold text-gray-800 mb-4" style="flex-shrink: 0;">
                ...
                Trend Inject In vs Out (7 Hari Terakhir)
            </h3>
            <div style="flex: 1; position: relative; min-height: 0; overflow: hidden;">
                <canvas id="injectInOutChart"></canvas>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6" style="height: 400px; display: flex; flex-direction: column;">
            <h3 class="text-lg font-semibold text-gray-800 mb-4" style="flex-shrink: 0;">
                ...
                Manpower Inject (7 Hari Terakhir)
            </h3>
            <div style="flex: 1; position: relative; min-height: 0; overflow: hidden;">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6" style="height: 400px; display: flex; flex-direction: column;">
            <h3 class="text-lg font-semibold text-gray-800 mb-4" style="flex-shrink: 0;">
                ...
                Inject per Mesin (7 Hari Terakhir)
            </h3>
            <div style="flex: 1; position: relative; min-height: 0; overflow: hidden;">
                <canvas id="injectPerMesinChart"></canvas>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6" style="height: 400px; display: flex; flex-direction: column;">
            <h3 class="text-lg font-semibold text-gray-800 mb-4" style="flex-shrink: 0;">
                ...
                Top 5 Part Paling Banyak Diproduksi (7 Hari Terakhir)
            </h3>
            <div style="flex: 1; position: relative; min-height: 0; overflow: hidden;">
...
function initCharts() {
    ...
    const rawInjectInData = {!! json_encode($injectInChartData ?? []) !!};
    const rawInjectOutData = {!! json_encode($injectOutChartData ?? []) !!};
    const rawManpowerData = {!! json_encode($manpowerChartData ?? []) !!};
    const injectPerMesin = {!! json_encode($injectPerMesin ?? []) !!};
    ...
    let injectInData = Array.isArray(rawInjectInData) && rawInjectInData.length > 0 ? rawInjectInData : [0, 0, 0, 0, 0, 0, 0];
    let injectOutData = Array.isArray(rawInjectOutData) && rawInjectOutData.length > 0 ? rawInjectOutData : [0, 0, 0, 0, 0, 0, 0];
    let manpowerData = Array.isArray(rawManpowerData) && rawManpowerData.length > 0 ? rawManpowerData : [0, 0, 0, 0, 0, 0, 0];

    // Chart 1: Inject In vs Out
    const canvas1 = document.getElementById('injectInOutChart');
    if (!canvas1) {
        console.error('Canvas injectInOutChart tidak ditemukan!');
        return;
    }
    ...
    // Chart 3: Inject per Mesin
    const injectPerMesinLabels = Object.keys(injectPerMesin).length > 0 ? Object.keys(injectPerMesin) : ['Belum ada data'];
    const injectPerMesinValues = Object.values(injectPerMesin).length > 0 ? Object.values(injectPerMesin) : [1];
    const canvas3 = document.getElementById('injectPerMesinChart');
    if (canvas3) {
        const ctx3 = canvas3.getContext('2d');
        new Chart(ctx3, {
            type: 'doughnut',
            data: {
                labels: injectPerMesinLabels,
                datasets: [{
                    data: injectPerMesinValues,
                    backgroundColor: [
                        'rgba(99, 102, 241, 0.8)',
                        'rgba(16, 185, 129, 0.8)',
                        'rgba(245, 158, 11, 0.8)',
