...
<div class="fade-in">
    ...
    <div class="mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-900 leading-none">Import Plant Gate Part</h2>
        <p class="text-[10px] text-gray-500 mt-1.5 uppercase font-bold tracking-wider">Import relasi plant gate dengan part dari file CSV dengan mapping kolom dinamis</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <form action="{{ route('submaster.plantgatepart.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">Contoh: Astra Honda Motor, PT</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">Contoh: Gate A</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">Contoh: 64301-K25-N00</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('submaster.plantgatepart.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-3 rounded-lg transition-colors font-semibold">
                    Batal
                </a>
            </div>
        </form>
    ...
    <div class="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        ...
        <ul class="text-sm text-yellow-800 space-y-2">
            <li class="flex items-start gap-2">
                ...
                <span>Customer dan Plant Gate harus <strong>sudah ada</strong> di Master Plant Gate</span>
            </li>
            <li class="flex items-start gap-2">
                <span class="text-yellow-600 mt-0.5">•</span>
                <span>Nomor Part harus <strong>sudah ada</strong> di Submaster Part</span>
            </li>
            <li class="flex items-start gap-2">
                <span class="text-yellow-600 mt-0.5">•</span>
                <span>Data duplikat akan dilewati (tidak error)</span>
            </li>
            <li class="flex items-start gap-2">
                <span class="text-yellow-600 mt-0.5">•</span>
...
document.querySelectorAll('input[name^="col_"]').forEach(input => {
    // Load saved value
    const saved = localStorage.getItem(`plantgatepart_${input.name}`);
    if (saved && !input.value) input.value = saved;
    
    // Save on change
    input.addEventListener('change', () => {
        localStorage.setItem(`plantgatepart_${input.name}`, input.value.toUpperCase());
    });
});
</script>
@endsection
