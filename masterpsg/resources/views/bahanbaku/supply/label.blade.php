...
<div class="no-print" style="display:flex;gap:8px;align-items:center;justify-content:space-between;margin-bottom:12px;">
  <div>
    <div style="font-weight:700;font-size:16px;">Label Supply (90x60mm)</div>
    <div style="color:#555;font-size:12px;">
      Tanggal: {{ optional($supply->tanggal_supply)->format('Y-m-d') }} |
      Tujuan: {{ strtoupper($supply->tujuan) }} |
      @if($supply->planningRun)
        Run: #{{ $supply->planning_run_id }} | 
        Mesin: {{ $supply->planningRun->day->mesin->no_mesin ?? '-' }} |
        Part: {{ $supply->planningRun->mold->part->nomor_part ?? '-' }}
      @else
        ASSY
      @endif
    </div>
...
<div id="labelsRoot">
  @php
    $month = (int) optional($supply->tanggal_supply)->format('n');
    $lotNumber = $supply->details->first()->lot_number ?? '-';
    
    // Ambil nomor mesin
    $noMesin = '-';
    if ($supply->planningRun && $supply->planningRun->day && $supply->planningRun->day->mesin) {
      $noMesin = $supply->planningRun->day->mesin->no_mesin ?? '-';
    }
    
    // Ambil nomor part (untuk ASSY atau dari planning run)
    $nomorPart = '-';
    if ($supply->part) {
      $nomorPart = $supply->part->nomor_part ?? '-';
    } elseif ($supply->planningRun && $supply->planningRun->mold && $supply->planningRun->mold->part) {
      $nomorPart = $supply->planningRun->mold->part->nomor_part ?? '-';
    }
    
    // Kumpulkan kategori unik dari semua detail
    $kategoris = [];
    foreach ($supply->details as $d) {
      $receivingDetail = $d->receivingDetail;
    ...
    // Buat komposisi dengan inisial
    $komposisi = [];
    if (in_array('material', $kategoris)) {
      $komposisi[] = 'M';
    }
    if (in_array('masterbatch', $kategoris)) {
      $komposisi[] = 'MB';
    }
    if (in_array('subpart', $kategoris)) {
      $komposisi[] = 'SB';
    }
    $komposisiStr = !empty($komposisi) ? implode('/', $komposisi) : '-';
    
    // Tujuan text
    $tujuanText = strtoupper($supply->tujuan);
    if ($supply->tujuan === 'inject' && $noMesin !== '-') {
      $tujuanText = $tujuanText . ' - MESIN ' . $noMesin;
    } elseif ($supply->tujuan === 'assy') {
      $meja = $supply->meja ?? '-';
      if ($nomorPart !== '-') {
        $tujuanText = $tujuanText . ' - MEJA ' . $meja;
      } else {
        $tujuanText = $tujuanText . ' - MEJA ' . $meja;
      }
    }
  @endphp
  <div class="label-page" id="qr-supply-{{ $supply->id }}" data-lot-number="{{ $lotNumber }}">
    <table class="label-table">
      ...
      <tr>
        ...
        <td class="center titleBox" colspan="2">LABEL SUPPLY</td>
      </tr>
      <tr>
        <td colspan="2">
          <table class="inner-table info-block">
            <tr>
              <td style="width:26mm;" class="label">TUJUAN</td>
              <td style="width:3mm;">:</td>
              <td class="value">{{ $tujuanText }}</td>
            </tr>
            <tr>
              <td class="label">Komposisi</td>
              <td>:</td>
              <td>
                <span class="value">{{ $komposisiStr }}</span>
              </td>
            </tr>
            <tr>
              <td class="label">TANGGAL SUPPLY</td>
              <td>:</td>
              <td class="value">{{ optional($supply->tanggal_supply)->format('d/m/y') }}</td>
            </tr>
            <tr>
              <td class="label">LOT NUMBER</td>
              <td>:</td>
              <td class="mono value" style="word-break:break-all;">{{ $lotNumber }}</td>
            </tr>
          </table>
        </td>
          <td rowspan="3" style="padding:0;">
            <table class="label-table months" style="width:100%;height:100%;border-collapse:collapse;">
              <tr>
                <th colspan="2" class="center">BULAN SUPPLY</th>
              </tr>
              <tr>
...
(function(){
  ...
  const cards = Array.from(root.querySelectorAll('[data-lot-number]'));
  for (const card of cards) {
    const text = card.getAttribute('data-lot-number') || '';
