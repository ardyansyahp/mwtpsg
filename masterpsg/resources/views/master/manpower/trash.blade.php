...
<div class="fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <form action="{{ route('master.manpower.empty.trash') }}" method="POST" onsubmit="return confirm('APAKAH ANDA YAKIN? Semua data di tong sampah akan dihapus PERMANEN dan tidak bisa dikembalikan.');">
            @csrf
            <button 
                type="submit" 
    ...
    @if($manpowers->count() > 0 && userCan('master.manpower.delete'))
    <div class="mb-4">
         <form action="{{ route('master.manpower.restore.all') }}" method="POST" onsubmit="return confirm('Pulihkan SEMUA data di tong sampah?');">
            @csrf
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($manpowers as $index => $manpower)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $manpower->deleted_at->format('d M Y H:i') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex justify-center gap-2">
                                {{-- Restore --}}
                                <form action="{{ route('master.manpower.restore', $manpower->id) }}" method="POST">
                                    @csrf
                                ...
                                {{-- Force Delete --}}
                                <form action="{{ route('master.manpower.force.delete', $manpower->id) }}" method="POST" onsubmit="return confirm('Hapus permanen? Data tidak bisa kembali.');">
                                    @csrf
