...
<div class="fade-in">
    <div class="mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-900 leading-none">Edit Plant Gate</h2>
        <p class="text-[10px] text-gray-500 mt-1.5 uppercase font-bold tracking-wider">Edit data plant gate</p>
    </div>

    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form id="editForm" class="space-y-6">
            @csrf
            {{-- REMOVED @method('PUT') because route is POST --}}
            ...
            <div>
                ...
                <select id="customer_id" name="customer_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    @foreach($customers as $customer)
                        <option value="{{ $customer->id }}" {{ $plantgate->customer_id == $customer->id ? 'selected' : '' }}>{{ $customer->nama_perusahaan }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" id="nama_plantgate" name="nama_plantgate" required maxlength="255" value="{{ $plantgate->nama_plantgate }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Masukkan nama plant gate">
            </div>
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const response = await fetch(`/master/plantgate/${plantgateId}/update`, {
                    method: 'POST',
                    body: formData,
                    headers: { 
