...
<div class="fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3">
            <div>
                <h2 class="text-xl font-bold text-gray-900 leading-none">Master Plant Gate</h2>
                <p class="text-[10px] text-gray-500 mt-1.5 uppercase font-bold tracking-wider">Kelola data plant gate customer</p>
            </div>
        </div>
        
        <div class="flex flex-col md:flex-row gap-2 items-start md:items-center">
            {{-- Search Form --}}
            <form action="{{ route('master.plantgate.index') }}" method="GET" class="flex gap-2">
                ...
                {{-- Filter Customer --}}
                <select name="customer_filter" class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" onchange="this.form.submit()">
                    <option value="">Semua Customer</option>
                    @foreach($customers as $cust)
                        <option value="{{ $cust->id }}" {{ request('customer_filter') == $cust->id ? 'selected' : '' }}>
                ...
                @if(request('search') || request('sort_by') || request('customer_filter'))
                    <a href="{{ route('master.plantgate.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg transition-colors border border-gray-300" title="Reset Filters">
            ...
            <div class="flex items-center gap-2">
                @if(userCan('master.plantgate.delete'))
                    ...
                    <div class="flex items-center bg-white rounded-lg border border-gray-300 shadow-sm p-1 ml-2">
                        {{-- Reset Database --}}
                        <form action="{{ route('master.plantgate.destroy.all') }}" method="POST" onsubmit="return confirm('EXTREME DANGER: Hapus SEMUA data?');" class="flex">
                            @csrf
                        ...
                        {{-- Recycle Bin --}}
                        <a href="{{ route('master.plantgate.trash') }}" class="p-2 text-gray-600 hover:bg-gray-100 rounded transition-colors" title="Recycle Bin">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                        </a>
                    </div>
                @endif
                ...
                @if(userCan('master.plantgate.create'))
                    ...
                    <div class="flex items-center gap-2 ml-2">
                        <a href="{{ route('master.plantgate.export') }}" target="_blank" class="flex items-center gap-2 bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors shadow-sm font-medium">
                        ...
                        <a href="{{ route('master.plantgate.import.form') }}" class="flex items-center gap-2 bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors shadow-sm font-medium">
                        ...
                        {{-- Primary Action --}}
                        <a href="{{ route('master.plantgate.create') }}" class="flex items-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700 transition-colors shadow-md font-bold ml-2">
    ...
    @php
        function sortLink($column, $label) {
            ...
            $url = route('master.plantgate.index', array_merge(request()->all(), ['sort_by' => $column, 'sort_order' => $direction]));
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {!! sortLink('nama_plantgate', 'Nama Plant Gate') !!}
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                    @forelse($plantgates as $index => $pg)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <input type="checkbox" name="selected_ids[]" value="{{ $pg->id }}" class="row-checkbox rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" onchange="updateBulkActionState()">
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $plantgates->firstItem() + $index }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $pg->customer->nama_perusahaan ?? '-' }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                <button
                                    onclick="toggleStatus({{ $pg->id }})"
                                    class="relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 {{ $pg->status ? 'bg-green-600' : 'bg-gray-200' }}"
                                    id="status-btn-{{ $pg->id }}"
                                    title="Klik untuk mengubah status"
                                >
                                    <span
                                        class="inline-block w-4 h-4 transform bg-white rounded-full transition-transform {{ $pg->status ? 'translate-x-6' : 'translate-x-1' }}"
                                        id="status-dot-{{ $pg->id }}"
                                    ></span>
                                </button>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('master.plantgate.show', $pg->id) }}" class="text-blue-600 hover:text-blue-900 transition-colors bg-blue-50 px-2 py-1 rounded-md border border-blue-200 hover:bg-blue-100" title="Lihat Detail">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    @if(userCan('master.plantgate.edit'))
                                    <a 
                                        href="{{ route('master.plantgate.edit', $pg->id) }}"
                                    ...
                                    @if(userCan('master.plantgate.delete'))
                                    <a 
                                        href="{{ route('master.plantgate.delete', $pg->id) }}"
                    ...
                    @empty
                        <tr id="emptyState">
                            <td colspan="7" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-1 text-sm text-gray-500">Mulai dengan menambahkan plant gate baru.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="bg-white px-6 py-4 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4" id="paginationInfo">
            <div class="flex items-center gap-4 text-sm text-gray-600 order-2 md:order-1">
                ...
                <div>
                    Menampilkan <span class="font-medium text-gray-900">{{ $plantgates->firstItem() ?? 0 }}</span> - <span class="font-medium text-gray-900">{{ $plantgates->lastItem() ?? 0 }}</span> dari <span class="font-medium text-gray-900">{{ $plantgates->total() }}</span>️ data
                </div>
            </div>
            <div class="order-1 md:order-2">
                {{ $plantgates->appends(request()->all())->links('vendor.pagination.custom') }}
            </div>
        </div>
    </div>
</div>
...
<script>
    function toggleStatus(id) {
        ...
        fetch(`{{ url('master/plantgate') }}/${id}/toggle-status`, {
            method: 'POST',
            headers: {
    ...
    function bulkDelete() {
        ...
        fetch('{{ route('master.plantgate.bulk.delete') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
