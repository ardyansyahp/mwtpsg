...
<div class="fade-in">
    ...
    <div class="mb-6">
        ...
        <h2 class="text-xl font-bold text-gray-900 leading-none">Import Master Kendaraan</h2>
        <p class="text-[10px] text-gray-500 mt-1.5 uppercase font-bold tracking-wider">Import data kendaraan dari file CSV dengan mapping kolom dinamis</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <form action="{{ route('master.kendaraan.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Nopol <span class="text-red-500">*</span>
                            </label>
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('master.kendaraan.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-3 rounded-lg transition-colors font-semibold">
                    Batal
                </a>
            </div>
        </form>
...
document.querySelectorAll('input[name^="col_"]').forEach(input => {
    // Load saved value
    const saved = localStorage.getItem(`kendaraan_${input.name}`);
    if (saved && !input.value) input.value = saved;
    
    // Save on change
    input.addEventListener('change', () => {
        localStorage.setItem(`kendaraan_${input.name}`, input.value.toUpperCase());
    });
});
</script>
@endsection
