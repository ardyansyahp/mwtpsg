...
<div class="fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex flex-col md:flex-row gap-2 items-start md:items-center">
            {{-- Search Form --}}
            <form action="{{ route('master.kendaraan.index') }}" method="GET" class="flex gap-2">
                ...
                <select name="jenis" class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" onchange="this.form.submit()">
                    ...
                    @foreach($types as $type)
                        <option value="{{ $type }}" {{ request('jenis') == $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
                ...
                @if(request('search') || request('sort_by') || request('jenis'))
                    <a href="{{ route('master.kendaraan.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg transition-colors border border-gray-300" title="Reset Filters">
            ...
            <div class="flex items-center gap-2">
                @if(userCan('master.kendaraan.delete'))
                    {{-- Maintenance Group --}}
                    <div class="flex items-center bg-white rounded-lg border border-gray-300 shadow-sm p-1 ml-2">
                        {{-- Reset Database --}}
                        <form action="{{ route('master.kendaraan.destroy.all') }}" method="POST" onsubmit="return confirm('EXTREME DANGER: Hapus SEMUA data?');" class="flex">
                            @csrf
                        ...
                        {{-- Recycle Bin --}}
                        <a href="{{ route('master.kendaraan.trash') }}" class="p-2 text-gray-600 hover:bg-gray-100 rounded transition-colors" title="Recycle Bin">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                        </a>
                    </div>
                @endif

                @if(userCan('master.kendaraan.create'))
                    {{-- Data Tools --}}
                    <div class="flex items-center gap-2 ml-2">
                        <a href="{{ route('master.kendaraan.export') }}" target="_blank" class="flex items-center gap-2 bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors shadow-sm font-medium">
                        ...
                        <a href="{{ route('master.kendaraan.import.form') }}" class="flex items-center gap-2 bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors shadow-sm font-medium">
                        ...
                        {{-- Primary Action --}}
                        <a href="{{ route('master.kendaraan.create') }}" class="flex items-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700 transition-colors shadow-md font-bold ml-2">
    ...
    @php
        function sortLink($column, $label) {
            ...
            $url = route('master.kendaraan.index', array_merge(request()->all(), ['sort_by' => $column, 'sort_order' => $direction]));
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {!! sortLink('nopol_kendaraan', 'Nopol') !!}
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {!! sortLink('jenis_kendaraan', 'Jenis') !!}
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {!! sortLink('merk_kendaraan', 'Merk') !!}
                        </th>
                         <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {!! sortLink('tahun_kendaraan', 'Tahun') !!}
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                    @forelse($kendaraans as $index => $vm)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <input type="checkbox" name="selected_ids[]" value="{{ $vm->id }}" class="row-checkbox rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" onchange="updateBulkActionState()">
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $kendaraans->firstItem() + $index }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $vm->created_at->format('d M Y') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                <button
                                    onclick="toggleStatus({{ $vm->id }})"
                                    class="relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 {{ $vm->status ? 'bg-green-600' : 'bg-gray-200' }}"
                                    id="status-btn-{{ $vm->id }}"
                                    title="Klik untuk mengubah status"
                                >
                                    <span
                                        class="inline-block w-4 h-4 transform bg-white rounded-full transition-transform {{ $vm->status ? 'translate-x-6' : 'translate-x-1' }}"
                                        id="status-dot-{{ $vm->id }}"
                                    ></span>
                                </button>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex items-center justify-center gap-2">
                                     <a href="{{ route('master.kendaraan.show', $vm->id) }}" class="text-green-600 hover:text-green-900 transition-colors bg-green-50 px-2 py-1 rounded-md border border-green-200 hover:bg-green-100" title="Detail & QR">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/></svg>
                                    </a>
                                    
                                    @if(userCan('master.kendaraan.edit'))
                                    <a 
                                        href="{{ route('master.kendaraan.edit', $vm->id) }}"
                                    ...
                                    @if(userCan('master.kendaraan.delete'))
                                    <a 
                                        href="{{ route('master.kendaraan.delete', $vm->id) }}"
        ...
        <div class="bg-white px-6 py-4 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4" id="paginationInfo">
            <div class="flex items-center gap-4 text-sm text-gray-600 order-2 md:order-1">
                ...
                <div>
                    Menampilkan <span class="font-medium text-gray-900">{{ $kendaraans->firstItem() ?? 0 }}</span> - <span class="font-medium text-gray-900">{{ $kendaraans->lastItem() ?? 0 }}</span> dari <span class="font-medium text-gray-900">{{ $kendaraans->total() }}</span> data
                </div>
            </div>
            <div class="order-1 md:order-2">
                {{ $kendaraans->appends(request()->all())->links('vendor.pagination.custom') }}
            </div>
        </div>
    </div>
</div>
...
<script>
    function toggleStatus(id) {
        ...
        fetch(`{{ url('master/kendaraan') }}/${id}/toggle-status`, {
            method: 'POST',
            headers: {
    ...
    function bulkDelete() {
        ...
        fetch('{{ route('master.kendaraan.bulk.delete') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
