...
<div class="fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <form action="{{ route('master.mesin.empty.trash') }}" method="POST" onsubmit="return confirm('APAKAH ANDA YAKIN? Semua data di tong sampah akan dihapus PERMANEN dan tidak bisa dikembalikan.');">
            @csrf
            <button 
                type="submit" 
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6 flex items-start gap-3">
        ...
        <div>
            <h3 class="font-semibold text-blue-900">Info Recycle Bin</h3>
            <p class="text-sm text-blue-800">Data yang ada di sini adalah data yang dihapus sementara (Soft Delete). Anda dapat memulihkannya kembali ke daftar aktif atau menghapusnya secara permanen.</p>
        </div>
    </div>
    ...
    @if($mesins->count() > 0 && userCan('master.mesin.delete'))
    <div class="mb-4">
         <form action="{{ route('master.mesin.restore.all') }}" method="POST" onsubmit="return confirm('Pulihkan SEMUA data di tong sampah?');">
            @csrf
        ...
        <table class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">No Mesin</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($mesins as $index => $mesin)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $mesin->deleted_at->format('d M Y H:i') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex justify-center gap-2">
                                {{-- Restore --}}
                                <form action="{{ route('master.mesin.restore', $mesin->id) }}" method="POST">
                                    @csrf
                                ...
                                {{-- Force Delete --}}
                                <form action="{{ route('master.mesin.force.delete', $mesin->id) }}" method="POST" onsubmit="return confirm('Hapus permanen? Data tidak bisa kembali.');">
                                    @csrf
