...
<div class="fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="text-xl font-bold text-gray-900 leading-none">Master Mesin</h2>
            <p class="text-[10px] text-gray-500 mt-1.5 uppercase font-bold tracking-wider">Kelola data mesin produksi</p>
        </div>
        
        <div class="flex flex-col md:flex-row gap-2 items-start md:items-center">
            {{-- Search Form --}}
            <form action="{{ route('master.mesin.index') }}" method="GET" class="flex gap-2">
                ...
                <div class="relative">
                    <input 
                        ...
                        placeholder="Cari No/Merk/QR..." 
                        class="pl-9 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-48 md:w-64"
                    >
                    <button type="submit" class="absolute inset-y-0 left-0 pl-3 flex items-center group">
                        <svg class="h-5 w-5 text-gray-400 group-hover:text-blue-500 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                        </svg>
                    </button>
                </div>

                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-colors font-medium text-sm shadow-sm">
                    Cari
                </button>

                @if(request('search') || request('sort_by'))
                    <a href="{{ route('master.mesin.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg transition-colors border border-gray-300" title="Reset Filters">
            ...
            <div class="flex items-center gap-2">
                @if(userCan('master.mesin.delete'))
                    ...
                    <div class="flex items-center bg-white rounded-lg border border-gray-300 shadow-sm p-1 ml-2">
                        {{-- Reset Database --}}
                        <form action="{{ route('master.mesin.destroy.all') }}" method="POST" onsubmit="return confirm('EXTREME DANGER: Hapus SEMUA data mesin?');" class="flex">
                            @csrf
                        ...
                        {{-- Recycle Bin --}}
                        <a href="{{ route('master.mesin.trash') }}" class="p-2 text-gray-600 hover:bg-gray-100 rounded transition-colors" title="Recycle Bin">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                        </a>
                    </div>
                @endif
                ...
                @if(userCan('master.mesin.create'))
                    ...
                    <div class="flex items-center gap-2 ml-2">
                        <a href="{{ route('master.mesin.export') }}" target="_blank" class="flex items-center gap-2 bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors shadow-sm font-medium">
                        ...
                        <a href="{{ route('master.mesin.import.form') }}" class="flex items-center gap-2 bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors shadow-sm font-medium">
                        ...
                        {{-- Primary Action --}}
                        <a href="{{ route('master.mesin.create') }}" class="flex items-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700 transition-colors shadow-md font-bold ml-2">
    ...
    @php
        function sortLink($column, $label) {
            ...
            $url = route('master.mesin.index', array_merge(request()->all(), ['sort_by' => $column, 'sort_order' => $direction]));
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {!! sortLink('no_mesin', 'No Mesin') !!}
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {!! sortLink('merk_mesin', 'Merk') !!}
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            {!! sortLink('tonase', 'Tonase') !!}
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            QR Code
                        </th>
                    ...
                    @forelse($mesins as $index => $mesin)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <input type="checkbox" name="selected_ids[]" value="{{ $mesin->id }}" class="row-checkbox rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" onchange="updateBulkActionState()">
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                <button
                                    onclick="toggleStatus({{ $mesin->id }})"
                                    class="relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 {{ $mesin->status ? 'bg-green-600' : 'bg-gray-200' }}"
                                    id="status-btn-{{ $mesin->id }}"
                                    title="Klik untuk mengubah status"
                                >
                                    <span
                                        class="inline-block w-4 h-4 transform bg-white rounded-full transition-transform {{ $mesin->status ? 'translate-x-6' : 'translate-x-1' }}"
                                        id="status-dot-{{ $mesin->id }}"
                                    ></span>
                                </button>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex items-center justify-center gap-2">
                                     <a href="{{ route('master.mesin.show', $mesin->id) }}" class="text-blue-600 hover:text-blue-900 transition-colors bg-blue-50 px-2 py-1 rounded-md border border-blue-200 hover:bg-blue-100" title="Lihat Detail">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    @if(userCan('master.mesin.edit'))
                                    <a 
                                        href="{{ route('master.mesin.edit', $mesin->id) }}"
                                    ...
                                    @endif
                                    <a href="{{ route('master.mesin.idcard', $mesin->id) }}" class="text-green-600 hover:text-green-900" title="ID Card" target="_blank">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2"/></svg>
                                    </a>
                                    @if(userCan('master.mesin.delete'))
                                    <a 
                                        href="{{ route('master.mesin.delete', $mesin->id) }}"
                    ...
                    @empty
                        <tr id="emptyState">
                            <td colspan="8" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-1 text-sm text-gray-500">Mulai dengan menambahkan mesin baru.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="bg-white px-6 py-4 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4" id="paginationInfo">
            <div class="flex items-center gap-4 text-sm text-gray-600 order-2 md:order-1">
                ...
                <div>
                    Menampilkan <span class="font-medium text-gray-900">{{ $mesins->firstItem() ?? 0 }}</span> - <span class="font-medium text-gray-900">{{ $mesins->lastItem() ?? 0 }}</span> dari <span class="font-medium text-gray-900">{{ $mesins->total() }}</span>️ data
                </div>
            </div>
            <div class="order-1 md:order-2">
                {{ $mesins->links('vendor.pagination.custom') }}
            </div>
        </div>
    </div>
</div>
...
<script>
    function toggleStatus(id) {
        if(!confirm('Apakah Anda yakin ingin mengubah status mesin ini?')) return;
        ...
        fetch(`{{ url('master/mesin') }}/${id}/toggle-status`, {
            method: 'POST',
            headers: {
    ...
    function bulkDelete() {
        ...
        fetch('{{ route('master.mesin.bulk.delete') }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json'
