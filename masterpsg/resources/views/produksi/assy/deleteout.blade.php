...
<div class="fade-in">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 max-w-md mx-auto">
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-800">Hapus ASSY Out</h2>
        ...
        <div class="bg-gray-50 rounded-lg p-4 mb-6">
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="font-mono font-medium text-gray-800">{{ $assyOut->lot_number ?? '-' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Waktu Scan Out:</span>
                    <span class="font-medium text-gray-800">{{ optional($assyOut->waktu_scan)->format('Y-m-d H:i:s') }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Part:</span>
                    <span class="font-medium text-gray-800">
                        @if($assyOut->part)
                            {{ $assyOut->part->nomor_part }}
                        @elseif($assyOut->assyIn && $assyOut->assyIn->part)
                            {{ $assyOut->assyIn->part->nomor_part }}
                        @else
                            -
                        @endif
                    </span>
...
<script>
(function() {
    const form = document.getElementById('deleteAssyOutForm');
    ...
    form?.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('{{ route("produksi.assy.destroyout", $assyOut->id) }}', {
                method: 'POST',
                body: formData,
                headers: {
