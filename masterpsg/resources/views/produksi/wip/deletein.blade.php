        ...
        <div class="bg-gray-50 rounded-lg p-4 mb-6">
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="font-mono font-medium text-gray-800">{{ $wipIn->lot_number ?? '-' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Waktu Scan In:</span>
                    <span class="font-medium text-gray-800">{{ optional($wipIn->waktu_scan_in)->format('Y-m-d H:i:s') }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Box Number:</span>
                    <span class="font-medium text-gray-800">Box #{{ $wipIn->box_number ?? '-' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-gray-600">Planning Run:</span>
                    <span class="font-medium text-gray-800">#{{ $wipIn->planning_run_id ?? '-' }}</span>
                </div>
                @if($wipIn->planningRun && $wipIn->planningRun->mold && $wipIn->planningRun->mold->part)
                    <div class="flex justify-between">
                        <span class="text-gray-600">Part:</span>
                        <span class="font-medium text-gray-800">{{ $wipIn->planningRun->mold->part->nomor_part ?? '-' }}</span>
                    </div>
                @endif
            </div>
        </div>

        <form id="deleteWipInForm">
            @csrf
            @method('DELETE')
...
<script>
(function() {
    const form = document.getElementById('deleteWipInForm');
    ...
    form?.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('{{ route("produksi.wip.destroyin", $wipIn->id) }}', {
                method: 'POST',
                body: formData,
                headers: {
