...
<div class="fade-in">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-3xl font-bold text-gray-800">Produksi WIP - Scan Out</h2>
            <p class="text-gray-600 mt-1">Scan label box yang sudah di-scan in untuk monitoring waktu keluar dari area WIP</p>
        </div>
        @if(userCan('produksi.wip.create'))
        <a 
            href="{{ route('produksi.wip.createout') }}" 
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                    @forelse($wipOuts as $index => $wipOut)
                        @php
                            $planningRun = $wipOut->planningRun;
                            $part = $planningRun && $planningRun->mold ? $planningRun->mold->part : null;
                            $wipIn = $wipOut->wipIn;
                            $durasi = null;
                            if ($wipIn && $wipIn->waktu_scan_in && $wipOut->waktu_scan_out) {
                                $durasi = $wipIn->waktu_scan_in->diffInMinutes($wipOut->waktu_scan_out);
                            }
                            ...
                            $scannedBoxCount = $wipOut->details->count();
                            $percentage = $targetBoxCount > 0 ? ($scannedBoxCount / $targetBoxCount) * 100 : 0;
                            $widthPercentage = min(100, max(0, $percentage));
                        @endphp
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $wipOuts->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $wipIn && $wipIn->waktu_scan_in ? $wipIn->waktu_scan_in->format('Y-m-d H:i:s') : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ optional($wipOut->waktu_scan_out)->format('Y-m-d H:i:s') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                @if($durasi !== null)
                                    <span class="px-2 py-1 rounded text-white bg-purple-600 font-medium">{{ $durasi }} menit</span>
                                @else
                                    <span class="text-gray-400">-</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-700">{{ $wipOut->lot_number ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <span class="px-2 py-1 rounded text-white bg-green-600 font-medium">Box #{{ $wipOut->box_number ?? '-' }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">#{{ $wipOut->planning_run_id ?? '-' }}</td>
                                    ...
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-2">
                                            <div class="flex-1 bg-gray-200 rounded-full h-2.5">
                                                @php
                                                    $progressColor = $percentage >= 100 ? 'bg-green-600' : ($percentage >= 80 ? 'bg-yellow-500' : 'bg-green-600');
                                                @endphp
                                                <div 
                                        ...
                                        @if($scannedBoxCount > 0)
                                            <div class="text-xs text-green-600 mt-1">
                                                Box: 
                                                @foreach($wipOut->details->sortBy('box_number') as $detail)
                                                    <span class="px-1.5 py-0.5 bg-green-100 rounded">#{{ $detail->box_number }}</span>
                                                @endforeach
                                            </div>
                                        @endif
                                    </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <a href="{{ route('produksi.wip.editout', $wipOut->id) }}" class="text-blue-600 hover:text-blue-900 transition-colors" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <a href="{{ route('produksi.wip.deleteout', $wipOut->id) }}" class="text-red-600 hover:text-red-900 transition-colors" title="Hapus">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @empty
                        <tr id="emptyState">
                            <td colspan="10" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-1 text-sm text-gray-500">Mulai dengan scan label box yang sudah di-scan in.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
