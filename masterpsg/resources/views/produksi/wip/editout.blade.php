...
<div class="fade-in">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route(\'dashboard\') }}"\1>
            ...
            <p class="text-gray-600 mt-1">Edit data scan out label box</p>
        </div>
    </div>

    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form id="editWipOutForm" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm text-gray-700">
                        {{ $wipOut->wipIn && $wipOut->wipIn->waktu_scan_in ? $wipOut->wipIn->waktu_scan_in->format('Y-m-d H:i:s') : '-' }}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm text-gray-700">
                        {{ optional($wipOut->waktu_scan_out)->format('Y-m-d H:i:s') }}
                    </div>
                </div>
                ...
                @if($wipOut->planningRun && $wipOut->planningRun->mold && $wipOut->planningRun->mold->part)
                    <div>
                        ...
                        <div class="px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm text-gray-700">
                            {{ $wipOut->planningRun->mold->part->nomor_part ?? '-' }}
                        </div>
                    </div>
                @endif
            </div>
...
<script>
(function() {
    const form = document.getElementById('editWipOutForm');
    ...
    form?.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('{{ route("produksi.wip.updateout", $wipOut->id) }}', {
                method: 'POST',
                body: formData,
                headers: {
