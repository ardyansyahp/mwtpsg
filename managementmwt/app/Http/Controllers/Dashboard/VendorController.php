...
class VendorController extends Controller
{
    public function index(Request $request)
    {
        ...
        $items = $bahanBakuList->map(function ($bahanBaku) use ($itemStats, $supplierGlobalStats) {
            ...
            // $supStat = $supplierGlobalStats->get($bahanBaku->supplier_id); // Unused variable

            // Item-specific Quality act uses item total act
            $quality_act = $itemStat ? $itemStat->total_act : 0;
            
            // Delivery stats use ITEM specific totals now
        ...
        if ($viewMode === 'daily') {
            ...
            // 2. Supplier Achievement (Selected Date)
            ...
            // 3. Category Achievement (Selected Date)
            ...
            $monthlyStats = TScheduleHeader::whereIn('periode', $months)
            ...
            $supplierStats = TScheduleHeader::where('periode', $currentMonth)
                ->join('m_perusahaan', 't_schedule_header.supplier_id', '=', 'm_perusahaan.id')
                ->select(
                    'm_perusahaan.inisial_perusahaan',
                    'm_perusahaan.nama_perusahaan',
            ...
            $categoryStats = TScheduleHeader::where('periode', $currentMonth)
                ->join('m_bahanbaku', 't_schedule_header.bahan_baku_id', '=', 'm_bahanbaku.id')
                ->select(
                    'm_bahanbaku.kategori', 
