...
<div class="fade-in">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form id="editFinishGoodInForm" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm text-gray-700">
                        {{ optional($finishGoodIn->waktu_scan)->format('Y-m-d H:i:s') }}
                    </div>
                </div>
                ...
                @if($finishGoodIn->assyOut && $finishGoodIn->assyOut->part)
                    <div>
                        ...
                        <div class="px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm text-gray-700">
                            {{ $finishGoodIn->assyOut->part->nomor_part }} - {{ $finishGoodIn->assyOut->part->nama_part }}
                        </div>
                    </div>
                @endif
                ...
                @if($finishGoodIn->assyOut)
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Waktu Scan Out ASSY</label>
                        <div class="px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm text-gray-700">
                            {{ optional($finishGoodIn->assyOut->waktu_scan)->format('Y-m-d H:i:s') }}
                        </div>
                    </div>
                @endif
            </div>
...
<script>
(function() {
    const form = document.getElementById('editFinishGoodInForm');
    ...
    form?.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('{{ route("finishgood.in.update", $finishGoodIn->id) }}', {
                method: 'POST',
                body: formData,
                headers: {
