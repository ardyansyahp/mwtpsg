    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <div class="flex items-center gap-3 mb-2">
                <a href="{{ route('master.mold.index') }}" class="text-gray-600 hover:text-gray-800 transition-colors">
        ...
        @if($trashed->count() > 0)
            <div class="flex gap-2">
                <form action="{{ route('master.mold.restore.all') }}" method="POST" onsubmit="return confirm('Pulihkan semua data?')">
                    @csrf
                ...
                <form action="{{ route('master.mold.empty.trash') }}" method="POST" onsubmit="return confirm('BAHAYA: Hapus permanen semua data? Tindakan ini tidak dapat dibatalkan!')">
                    @csrf
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($trashed as $index => $mold)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $mold->deleted_at ? $mold->deleted_at->format('d M Y H:i') : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button onclick="restore({{ $mold->id }})" class="text-green-600 hover:text-green-900 transition-colors" title="Pulihkan">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                                        </svg>
                                    </button>
                                    <button onclick="forceDelete({{ $mold->id }})" class="text-red-600 hover:text-red-900 transition-colors" title="Hapus Permanen">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                                    </button>
...
async function restore(id) {
    ...
    try {
        const response = await fetch(`/submaster/mold/${id}/restore`, {
            method: 'POST',
            headers: {
...
async function forceDelete(id) {
    ...
    try {
        const response = await fetch(`/submaster/mold/${id}/force-delete`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
