...
<div class="fade-in">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 max-w-md mx-auto">
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-800">Hapus ASSY In</h2>
        ...
        <div class="bg-gray-50 rounded-lg p-4 mb-6">
            <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="font-medium text-gray-800">{{ $assyIn->manpower ?? '-' }}</span>
                </div>
            </div>
        </div>

        <form id="deleteAssyInForm">
            @csrf
            @method('DELETE')
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route(\'dashboard\') }}" 
                    class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-lg transition-colors"
                >
                    Batal
                </a>
            </div>
...
<script>
(function() {
    const form = document.getElementById('deleteAssyInForm');
    ...
    form?.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('{{ route("produksi.assy.destroyin", $assyIn->id) }}', {
                method: 'POST',
                body: formData,
                headers: {
