...
<div class="fade-in">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form id="editForm" class="space-y-6">
            @csrf
            @method('PUT')
            <input type="hidden" id="mesin_id_hidden" value="{{ $mesin->id }}">
...
document.getElementById('editForm').addEventListener('submit', async function(e) {
    ...
    const mesinId = document.getElementById('mesin_id_hidden').value;
    ...
    try {
        // Use POST with _method PUT if needed, or just POST if the route is POST. 
        // My route is defined as POST /{mesin}/update
        formData.append('_method', 'POST'); 
        const response = await fetch(`{{ url('master/mesin') }}/${mesinId}/update`, {
            method: 'POST',
            body: formData,
            headers: {
