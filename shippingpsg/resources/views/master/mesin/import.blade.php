...
<div class="fade-in">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <form action="{{ route('master.mesin.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">Contoh: MC-01</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Merk Mesin <span class="text-red-500">*</span>
                            </label>
                            <div class="flex items-center gap-2">
                                <span class="bg-gray-100 text-gray-500 px-3 py-2 rounded-lg text-sm font-mono">Kolom</span>
                                <input 
                            ...
                            <p class="mt-1 text-xs text-gray-500">Contoh: Sumitomo</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Tonase <span class="text-red-500">*</span>
                            </label>
                            <div class="flex items-center gap-2">
                                <span class="bg-gray-100 text-gray-500 px-3 py-2 rounded-lg text-sm font-mono">Kolom</span>
                                <input 
                            ...
                            <p class="mt-1 text-xs text-gray-500">Contoh: 180</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('master.mesin.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-3 rounded-lg transition-colors font-semibold">
                    Batal
                </a>
            </div>
        </form>
    ...
    <div class="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        ...
        <ul class="text-sm text-yellow-800 space-y-2">
            <li class="flex items-start gap-2">
                ...
                <span>Data baru akan ditambahkan, tidak mengupdate data lama yang duplikat</span>
            </li>
            <li class="flex items-start gap-2">
                <span class="text-yellow-600 mt-0.5">•</span>
                <span>QR Code akan otomatis di-generate (No|Merk|Tonase)</span>
            </li>
            <li class="flex items-start gap-2">
                <span class="text-yellow-600 mt-0.5">•</span>
...
document.querySelectorAll('input[name^="col_"]').forEach(input => {
    // Load saved value
    const saved = localStorage.getItem(`mesin_${input.name}`);
    if (saved && !input.value) input.value = saved;
    
    // Save on change
    input.addEventListener('change', () => {
        localStorage.setItem(`mesin_${input.name}`, input.value.toUpperCase());
    });
});
</script>
@endsection
