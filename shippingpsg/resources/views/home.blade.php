...
@section('title', 'Shipping Command Center')
...
<div class="h-[calc(100vh-110px)] flex flex-col overflow-hidden px-4 pb-4 pt-1">
    ...
    <div class="mb-2 shrink-0" 
        ...
        <div class="relative w-full z-50">
            ...
            <input type="text" 
                   ...
                   class="block w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm placeholder-gray-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 shadow-sm transition-all" 
                   placeholder="Search Logistics Data (e.g. 'SJ-00123', 'Part Name', 'SPK-990')..."
                   autocomplete="off">
            ...
            <div x-show="showResults && (results.length > 0 || query.length >= 2)"
                    ...
                    <div>
                        ...
                        <ul class="divide-y divide-gray-50">
                            <template x-for="result in results">
                                <li>
                                    <a :href="result.url" class="block hover:bg-indigo-50/50 px-4 py-3 transition-colors">
                                        <div class="flex items-center">
                                            <div :class="`flex-shrink-0 h-8 w-8 rounded-lg bg-${result.icon === 'truck' ? 'indigo' : (result.icon === 'cube' ? 'blue' : 'amber')}-100 text-${result.icon === 'truck' ? 'indigo' : (result.icon === 'cube' ? 'blue' : 'amber')}-600 flex items-center justify-center`">
                                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                     <path x-show="result.icon === 'truck'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path>
                                            ...
                                            <div class="ml-3">
                                                ...
                                                <div class="flex items-center gap-2">
                                                    <span class="text-xs text-indigo-600 font-bold bg-indigo-50 px-1.5 rounded" x-text="result.category"></span>
                 ...
                 <div x-show="results.length === 0 && !loading" class="px-4 py-8 text-center text-gray-500">
                    ...
                    <p class="text-xs mt-1">Try a different keyword or document number.</p>
                 </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-2 shrink-0">
        @php
            $stockCount = class_exists('\App\Models\TStockFG') ? \App\Models\TStockFG::sum('qty') : 0;
            $spkCount = class_exists('\App\Models\TSpk') ? \App\Models\TSpk::whereNull('no_surat_jalan')->count() : 0;
            $deliveryToday = class_exists('\App\Models\TShippingDeliveryHeader') ? \App\Models\TShippingDeliveryHeader::whereDate('tanggal_berangkat', today())->count() : 0;
            $vehicleCount = class_exists('\App\Models\MKendaraan') ? \App\Models\MKendaraan::count() : 0;

            $kpis = [
                ['label' => 'Total Stock FG', 'value' => $stockCount, 'color' => 'blue', 'icon' => 'cube', 'can' => 'finishgood.stock.view'],
                ['label' => 'Active SPK', 'value' => $spkCount, 'color' => 'indigo', 'icon' => 'document', 'can' => 'spk.view'],
                ['label' => 'Delivery Today', 'value' => $deliveryToday, 'color' => 'emerald', 'icon' => 'truck', 'can' => 'shipping.delivery.view'],
                ['label' => 'Total Kendaraan', 'value' => $vehicleCount, 'color' => 'amber', 'icon' => 'key', 'can' => 'shipping.controltruck.monitoring']
            ];
        @endphp
        ...
        @foreach($kpis as $kpi)
            ...
            <div class="relative bg-white rounded-xl border border-{{ $kpi['color'] }}-100 p-3 flex items-center justify-between shadow-sm overflow-hidden group hover:border-{{ $kpi['color'] }}-300 transition-all hover:shadow-md">
                ...
                <div class="absolute -right-2 -bottom-2 text-{{ $kpi['color'] }}-100 transform rotate-12 opacity-20 group-hover:scale-110 transition-transform duration-500 pointer-events-none">
                     ...
                     @elseif($kpi['icon'] == 'key') <svg class="w-16 h-16" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-6-3a2 2 0 11-4 0 2 2 0 014 0zm-2 4a5 5 0 00-4.546 2.916A5.986 5.986 0 0010 16a5.986 5.986 0 004.546-2.084A5 5 0 0010 11z" clip-rule="evenodd" /></svg>
                     @endif
                </div>
                ...
                <div class="relative z-10 w-8 h-8 rounded-lg bg-{{ $kpi['color'] }}-50 text-{{ $kpi['color'] }}-600 flex items-center justify-center border border-{{ $kpi['color'] }}-100 shadow-sm group-hover:scale-110 transition-transform">
                     @if($kpi['icon'] == 'cube') <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
                     ...
                     @elseif($kpi['icon'] == 'key') <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg>
                     @endif
                </div>
            </div>
            @endif
    ...
    <div class="flex-1 flex flex-col lg:flex-row gap-4 min-h-0 overflow-hidden">
        ...
        <div class="flex-1 flex flex-col gap-4 min-w-0 overflow-hidden">
            ...
            <div class="flex-1 bg-white border border-gray-200 rounded-xl flex flex-col min-h-0 overflow-hidden shadow-sm">
                <div class="px-4 py-3 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between shrink-0">
                    <h3 class="text-[10px] font-black uppercase tracking-widest text-gray-600">Logistics Stream</h3>
                    <div class="flex items-center gap-2">
                        <span class="relative flex h-2 w-2">
                          <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span>
                          <span class="relative inline-flex rounded-full h-2 w-2 bg-indigo-500"></span>
                        </span>
                        <span class="text-[9px] font-bold text-indigo-600">LIVE UPDATES</span>
                    </div>
                </div>
                <div class="flex-1 overflow-y-auto p-0">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-50 text-[10px]">
                            @php
                                // 1. Fetch Latest Finish Good In (Scan In)
                                $fgIn = \App\Models\TFinishGoodIn::with('part')
                                    ->latest('waktu_scan')
                                    ->limit(5)
                                    ->get()
                                    ->map(function($item) {
                                        return [
                                            'time' => \Carbon\Carbon::parse($item->waktu_scan)->diffForHumans(),
                                            'timestamp' => \Carbon\Carbon::parse($item->waktu_scan),
                                            'activity' => 'Stock In: ' . ($item->part->nama_part ?? 'Unknown Part'),
                                            'ref' => 'Lot #' . $item->lot_number,
                                            'status' => 'STORED',
                                            'color' => 'emerald'
                                        ];
                                    });
                                ...
                                // 2. Fetch Latest SPKs
                                $spks = \App\Models\TSpk::latest('created_at')
                                    ->limit(5)
                                    ->get()
                                    ->map(function($item) {
                                        return [
                                ...
                                // 3. Fetch Latest Deliveries
                                $deliveries = \App\Models\TShippingDeliveryHeader::latest('updated_at')
                                    ->limit(5)
                                    ->get()
                                    ->map(function($item) {
                                        $statusMap = [
                                            'DRAFT' => ['label' => 'DRAFT', 'color' => 'gray'],
                                            'ON_ROUTE' => ['label' => 'ON ROUTE', 'color' => 'indigo'],
                                            'DELIVERED' => ['label' => 'DELIVERED', 'color' => 'emerald'],
                                            'COMPLETED' => ['label' => 'COMPLETED', 'color' => 'emerald'],
                                            'CANCELLED' => ['label' => 'CANCELLED', 'color' => 'red'],
                                        ];
                                        $status = $statusMap[$item->status] ?? ['label' => $item->status, 'color' => 'gray'];
                                        
                                        return [
                                            'time' => $item->updated_at->diffForHumans(),
                                            'timestamp' => $item->updated_at,
                                            'activity' => 'Delivery Update',
                                ...
                                // Merge and Sort
                                $activities = $fgIn->concat($spks)->concat($deliveries)
                            ...
                            <tr>
                                <td colspan="4" class="px-4 py-8 text-center text-gray-400 text-[10px] italic">
                                    No recent activities found. Start mainly operating to see updates here.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
        ...
        <div class="w-full lg:w-72 flex flex-col gap-4 shrink-0">
            {{-- User Profile Card --}}
            <div class="bg-indigo-950 rounded-xl p-5 text-white shadow-sm relative overflow-hidden shrink-0">
                <div class="relative z-10">
                    <div class="flex items-center gap-3 mb-4">
                        <div class="w-10 h-10 bg-indigo-800 rounded-lg flex items-center justify-center text-lg font-black border border-indigo-700">
                            {{ substr(session('user_name', 'U'), 0, 1) }}
                        </div>
                        <div>
                            <h3 class="text-xs font-black text-white leading-tight uppercase tracking-tight truncate w-32">{{ session('user_name', 'Logistics User') }}</h3>
                            <p class="text-[8px] text-indigo-300 font-bold uppercase tracking-wider mt-0.5">
                                Shipping Command
                            </p>
                        </div>
                    </div>
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full py-2 bg-indigo-900 hover:bg-indigo-800 text-indigo-200 hover:text-white rounded-lg border border-indigo-800 hover:border-indigo-700 transition-all text-[10px] font-black uppercase tracking-wider">
                            Logout
                        </button>
                    </form>
                </div>
                {{-- Decorative circles --}}
                <div class="absolute -right-4 -top-4 w-24 h-24 bg-indigo-800 rounded-full blur-2xl opacity-20"></div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 flex-1 flex flex-col min-h-0 overflow-hidden shadow-sm"
                 ...
                 x-init="runDiagnostic()">
                <div class="flex items-center justify-between mb-4 flex-shrink-0">
                    <h4 class="text-[10px] font-black text-gray-800 uppercase tracking-widest">Logistics Health</h4>
                ...
                <div class="mt-auto pt-3 border-t border-gray-50 flex-shrink-0 text-center">
                   <button @click="runDiagnostic()" 
                           :disabled="loading"
                           class="text-[9px] text-indigo-600 hover:text-indigo-800 font-bold uppercase tracking-wider disabled:opacity-50 flex items-center justify-center gap-2 w-full">
                        <span x-show="loading" class="animate-spin h-3 w-3 border-2 border-indigo-600 border-t-transparent rounded-full"></span>
                        <span x-text="loading ? 'Running Diagnostic...' : 'Run Logistics Check'"></span>
                   </button>
                </div>
            </div>
        </div>
