...
<div class="fade-in">
    ...
    <div class="mb-6">
        ...
        <h2 class="text-3xl font-bold text-gray-800">Import Stock Opname</h2>
        <p class="text-gray-600 mt-1">Update stok finish good dari file CSV dengan mapping kolom dinamis</p>
    </div>
        ...
        <form action="{{ route('stock.opname.import') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Qty Actual</label>
                        <div class="flex items-center gap-2">
                            <span class="bg-gray-100 text-gray-500 px-3 py-2 rounded-lg text-sm font-mono">Kolom</span>
                            <input type="text" name="col_qty_actual" placeholder="D" class="uppercase flex-1 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono" required>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <span class="bg-gray-100 text-gray-500 px-3 py-2 rounded-lg text-sm font-mono">Kolom</span>
                            <input type="text" name="col_keterangan" placeholder="E" class="uppercase flex-1 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="{{ route('stock.opname.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-3 rounded-lg transition-colors font-semibold">
                    Batal
                </a>
            </div>
        </form>
...
document.querySelectorAll('input[name^="col_"]').forEach(input => {
    // Load saved value
    const saved = localStorage.getItem(`stock_opname_${input.name}`);
    if (saved && !input.value) input.value = saved;
    
    // Save on change
    input.addEventListener('change', () => {
        localStorage.setItem(`stock_opname_${input.name}`, input.value.toUpperCase());
    });
});
</script>
@endsection
